(function ()
{
    ...
    angular
        ...
        .controller('BdbFinancialController', BdbFinancialController);

    function BdbFinancialController(fuseTheming,$mdSidenav,$timeout,$http,$scope,$location,commonUtils,apisUtils,usersApi)
    {
        ...
        function financialGraphs(graph){
            ...
            if(graph && graph.drybulk && angular.element('#dry-bulk-container').length){
                Highcharts.chart('dry-bulk-container', commonUtils.activityChart(graph.drybulk.graphInfo));
            }
        ...
        function financialTables(table){
            ...
            if(table && table.bdb_avg_financial_revenue) {

                var fkm_header_duration = commonUtils.categories(table.bdb_avg_financial_revenue.tableInfo.categories);
                var fkm_series_data     = commonUtils.series(table.bdb_avg_financial_revenue.tableInfo.series);
        ...
        function getServices(currentDuration){
            ...
            apisUtils.getBahriDryBulkFinancialPerformanceGraph(currentDuration)
                .then(function(response) {
                    if(vm.currentUrl == $location.absUrl()){
                        vm.graphInfo = response.bdb_financial_performance;
            ...
            apisUtils.getBahriDryBulkFinancialPerformanceTable(currentDuration)
                .then(function(response) {
                    if(vm.currentUrl == $location.absUrl()){
                        vm.tableInfo = response.bdb_financial_performance;
                        vm.date = response.current_date_time;
                        financialTables(vm.tableInfo);
                    }
                }, function(error) {
