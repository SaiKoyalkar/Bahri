(function ()
{
    ...
    angular
        ...
        .controller('BccFinancialController', BccFinancialController);

    function BccFinancialController(fuseTheming,$mdSidenav,$timeout,$http,$scope,$location,commonUtils,apisUtils,usersApi)
    {
        ...
        function financialGraphs(graph){
            ...
            if(graph && graph.chemical_tanker && angular.element('#chemical-tanker-container').length){
                Highcharts.chart('chemical-tanker-container', commonUtils.activityChart(graph.chemical_tanker.graphInfo));
            }
            if(graph && graph.chemical_product_tanker && angular.element('#chemical-product-tanker-container').length){
                Highcharts.chart('chemical-product-tanker-container', commonUtils.activityChart(graph.chemical_product_tanker.graphInfo));
            }
        ...
        function financialTables(table){
            ...
            if(table && table.bcc_avg_financial_revenue) {

                var fkm_header_duration = commonUtils.categories(table.bcc_avg_financial_revenue.tableInfo.categories);
                var fkm_series_data     = commonUtils.series(table.bcc_avg_financial_revenue.tableInfo.series);
            ...
            if(table && table.bcc_avg_running_cost) {

                var avg_ship_header_duration = commonUtils.categories(table.bcc_avg_running_cost.tableInfo.categories);
                var avg_ship_series_data     = commonUtils.series(table.bcc_avg_running_cost.tableInfo.series);
        ...
        function getServices(currentDuration){
            ...
            apisUtils.getBahriChemicalFinancialPerformanceGraph(currentDuration)
                .then(function(response) {
                    if(vm.currentUrl == $location.absUrl()){
                        vm.graphInfo = response.bcc_financial_performance;
            ...
            apisUtils.getBahriChemicalFinancialPerformanceTable(currentDuration)
                .then(function(response) {
                    if(vm.currentUrl == $location.absUrl()){
                        vm.tableInfo = response.bcc_financial_performance;
                        vm.date = response.current_date_time;
                        financialTables(vm.tableInfo);
                    }
                }, function(error) {
